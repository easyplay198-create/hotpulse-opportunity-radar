# PRAXON（派克森商机雷达）技术设计文档

## 1. 文档目标
本文档记录 PRAXON 当前技术设计边界，服务“AI驱动的出海商机发现与验证系统”这一产品定位。

PRAXON 当前不是旧版 `/api/hotspots` / `HotspotItem` / 内容热点评分体系。旧 `HotspotItem` / 内容热点评分模型不是当前核心语义。当前技术设计必须服务“AI驱动的出海商机发现与验证系统”。
