...
import { IntentUnderstandingCard } from './IntentUnderstandingCard';
...
export function AnalyzeResultSummary({ result, source, onReset, onRetry }: Props) {
  ...
  const noMatch = result.matchedOpportunities.length === 0;
  return (
    <section className={styles.resultStack}>
      <IntentUnderstandingCard parsedIntent={parsedIntent} />
      <ClarifyingQuestionsPanel questions={result.clarifyingQuestions} />
      {result.evidenceGaps?.length ? (
        <section className={styles.resultCard}>
          <h2>证据缺口</h2>
          <ul className={styles.planList}>{result.evidenceGaps.map((gap) => <li key={gap}>{gap}</li>)}</ul>
        </section>
      ) : null}
      ...
      {noMatch ? <NoMatchState evidenceGaps={result.evidenceGaps} rejectedSignals={result.relevanceScores?.rejectedSignals} nextStep={result.recommendation.nextStep} /> : null}
    </section>
  );
}
